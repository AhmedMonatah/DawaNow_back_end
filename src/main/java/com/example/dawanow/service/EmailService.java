...
public class EmailService {
    ...
    public void sendPasswordResetEmail(String toEmail, String name, String otp) {
        try {
            logger.info("Attempting to send Password Reset email to: {} via Resend API", toEmail);
            ...
            requestBody.put("from", "DawaNow <" + fromEmail + ">");
            requestBody.put("to", List.of(toEmail));
            requestBody.put("subject", "DawaNow - Password Reset");
            requestBody.put("html", htmlMsg);
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                logger.info("Password Reset email sent successfully via Resend API to: {}", toEmail);
            } else {
                logger.error("Failed to send email via Resend API. Status: {}, Body: {}", response.getStatusCode(), response.getBody());
                throw new EmailSendingException("Failed to send password reset email");
            }
        } catch (Exception e) {
            logger.error("Unexpected error sending Password Reset email via Resend to: {}. Error: {}", toEmail, e.getMessage(), e);
